...
export class GdmLiveAudioVisuals3D extends LitElement {
  ...
  private init() {
    ...
    scene.background = new THREE.Color(0x000510); // Deep Blue Black
    ...
    const geometry = new THREE.IcosahedronGeometry(1, 30); // Higher detail

    const pmremGenerator = new THREE.PMREMGenerator(renderer);
    pmremGenerator.compileEquirectangularShader();

    // Lighting setup instead of EXR loader
    ...
    const blueLight = new THREE.PointLight(0x0044aa, 5, 20);
    blueLight.position.set(-5, -2, 0);
    scene.add(blueLight);

    // Smart City Aesthetic: Cyan/Blue/Purple
  ...
  protected firstUpdated() {
    // Ensure we use renderRoot to query canvas
